...
def main():
    ...
    output_size = 8 + buffer_size*(12+8)
        ...
        with torch.inference_mode(): # step 2 and 3 (and 4 but not required) in inference mode to avoid gradient computations
            ...
            for j in range(trajectory_length_iter):
                ...
                for i in range(buffer_size): 
                    ...
                    p = student_actions[:,(f_len+d_len)                  :(f_len+d_len+buffer_size*p_len)        ].reshape(p_shape) # shape (num_envs, 4, 2, buffer_size)
                    F = student_actions[:,(f_len+d_len+buffer_size*p_len):(f_len+d_len+buffer_size*(p_len+F_len))].reshape(F_shape) # shape (num_envs, 4, 3, buffer_size)
